...
const Search = ({ socket }) => {
    ...
    const updateList = (search) => {
        ...
        fetch(`https://choice-champ-backend.vercel.app/media/${collectionType}/${search}/1`)
        .then(res => res.json())
        .then(res => {
    ...
    const removeItem = (itemId, board) => {
        ...
        fetch(`https://choice-champ-backend.vercel.app/collections/items/${collectionId}/${collectionItem.mongoId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            }
